...
router.get("/avaliacao-config", async (req, res) => {
  ...
  try {
    ...
    // Resolve exception IDs → names para o frontend poder comparar por nome de disciplina
    // sem precisar de chamada extra ou lógica de IDs
    let excecoes_nomes = [];
    const adotaBimestral = config["escola.avaliacao_padrao_bimestral"] === "1";
    if (adotaBimestral) {
      try {
        const excIds = JSON.parse(config["escola.avaliacao_padrao_bimestral.excecoes"] || "[]");
        if (Array.isArray(excIds) && excIds.length > 0) {
          ...
          excecoes_nomes = disciplinasExc.map(d => String(d.nome).trim().toLowerCase());
        }
      } catch (e) {
        console.warn("[GOVERNANCA][AVALIACAO-CONFIG] Erro ao resolver nomes de exceções:", e.message);
      }
    }

    return res.json({ ok: true, config, excecoes_nomes });
  } catch (err) {
    if (err?.code === "ER_NO_SUCH_TABLE") {
      return res.json({ ok: true, config: { ...AVALIACAO_DEFAULTS }, excecoes_nomes: [] });
    }
...
// ── GET /api/governanca/plano-modo?escola_id=X ─────────────────
// Retorna o modo ativo de governança de Planos de Avaliação (PAP).
// Modos:
//   "coordenacao_decide"    → Só direção/coordenação cria
//   "professor_aprovacao"   → Professor cria e envia para aprovação
//   "professor_autonomo"    → Professor cria com aprovação/publicação automática
// ──────────────────────────────────────────────────────────────────
router.get("/plano-modo", async (req, res) => {
  const db = req.db;
  const escolaId = Number(req.query.escola_id || req.user?.escola_id);
  if (!escolaId)
    return res.status(400).json({ ok: false, message: "escola_id é obrigatório." });
  ...
  const DEFAULTS = {
    direcao_decide:     "0",
    coordenador_decide: "1", // Padrão: Professor cria e coordenação aprova
    professor_decide:   "0",
  };
  ...
  try {
    ...
    // Determina modo ativo (prioridade: professor_autonomo > professor_aprovacao > coordenacao_decide)
    let modo = "coordenacao_decide"; // se nada for true ou se direcao_decide for true
    if (isOn("coordenador_decide")) modo = "professor_aprovacao";
    if (isOn("professor_decide"))   modo = "professor_autonomo";

    return res.json({ ok: true, modo, config: cfg });
  } catch (err) {
    if (err?.code === "ER_NO_SUCH_TABLE") {
      return res.json({ ok: true, modo: "professor_aprovacao", config: { ...DEFAULTS } });
    }
    console.error("[GOVERNANCA][PLANO-MODO]", err);
    return res.status(500).json({ ok: false, message: "Erro ao buscar modo de plano de avaliação." });
  }
});
  ...
  try {
    ...
    for (const plano of planos) {
      ...
      if (adotaBimestral && !isException) {
        ...
        // Busca item fixo_direcao=1 OU item que era fixo e foi zerado (fixo_direcao=0, atividade='Prova Bimestral')
        // Isso cobre o caso onde a disciplina foi exceção temporariamente e agora voltou ao padrão
        const [[jaExiste]] = await db.query(
          `SELECT id, fixo_direcao FROM itens_avaliacao
           WHERE plano_id = ? AND (fixo_direcao = 1 OR atividade = 'Prova Bimestral')
           LIMIT 1`,
          [planoId]
        );

        if (!jaExiste) {
          // Não existe nenhuma "Prova Bimestral" → criar do zero
          await db.query(
        ...
        } else if (jaExiste.fixo_direcao !== 1) {
          // Existe mas estava com fixo_direcao=0 (era exceção antes) → restaurar o lock
          await db.query(
            `UPDATE itens_avaliacao
             SET atividade = 'Prova Bimestral', tipo_avaliacao = 'PROVA', fixo_direcao = 1
             WHERE id = ?`,
            [jaExiste.id]
          );
          console.log(`[GOVERNANCA][SYNC-PAPs] Restaurado fixo_direcao=1 na Prova Bimestral do plano ${planoId} (disciplina saiu das exceções)`);
        } else {
          // Já existe e está correto — apenas garante nome e tipo
          await db.query(
            `UPDATE itens_avaliacao
        ...
        // --- NÃO deve ter Prova Bimestral (escola inativa ou disciplina é exceção) ---
        const [[jaExiste]] = await db.query(
        ...
        if (jaExiste) {
          ...
          if (idx !== -1) {
            ...
            if (count_notas === 0) {
              // Sem notas: pode deletar com segurança
              await db.query(
                `DELETE FROM itens_avaliacao WHERE id = ?`,
                [jaExiste.id]
              );
              console.log(`[GOVERNANCA][SYNC-PAPs] Removida Prova Bimestral do plano ${planoId} (exceção — sem notas)`);
            } else {
              // COM notas lançadas: NÃO é seguro deletar.
              // Remove apenas o flag fixo_direcao → coluna fica editável para o professor,
              // mas o item e todas as notas históricas são preservados.
              await db.query(
                `UPDATE itens_avaliacao SET fixo_direcao = 0 WHERE id = ?`,
                [jaExiste.id]
              );
              console.log(`[GOVERNANCA][SYNC-PAPs] Plano ${planoId}: disciplina agora é exceção mas já possui ${count_notas} nota(s). fixo_direcao=0 aplicado — dados preservados, coluna desbloqueada para o professor.`);
            }
          }
        }
      }
...
// ── POST /api/governanca/sync-paps — Força sincronização manual dos planos ──
// Permite à direção disparar o sync a qualquer momento sem alterar configurações
router.post("/sync-paps", guardDiretor, async (req, res) => {
  const db = req.db;
  const escolaId = Number(req.query.escola_id || req.user?.escola_id || req.headers["x-escola-id"]);
  ...
  // Dispara sync em segundo plano
  syncPlanosAvaliacao(db, escolaId).catch(err => {
    console.error("[GOVERNANCA][SYNC-MANUAL] Erro:", err);
  });

  return res.json({
    ok: true,
    message: "Sincronização de planos iniciada em segundo plano. Aguarde alguns segundos e recarregue.",
  });
});
