...
// ============================================================
// CRUD para configuracao de grade horaria por escola
// Suporte multi-escola: cada escola tem seus proprios turnos,
// dias letivos e numero de periodos por turno.
// ============================================================
...
// GET /api/escola/configuracao-grade
// Retorna a configuracao atual da escola do usuario logado
router.get('/', async (req, res) => {
  try {
    const { escola_id } = req.user;
    const [[row]] = await pool.query(
      `SELECT id, turnos, dias_semana, periodos, criado_em, atualizado_em
       FROM escola_configuracao_grade
       WHERE escola_id = ? LIMIT 1`,
      [escola_id]
    );
...
// POST /api/escola/configuracao-grade
// Cria ou atualiza a configuracao da escola
router.post('/', async (req, res) => {
  try {
    ...
    if (!Array.isArray(dias_semana) || dias_semana.length === 0)
      return res.status(400).json({ message: 'Informe os dias letivos.' });
    if (!periodos || typeof periodos !== 'object')
      return res.status(400).json({ message: 'Informe os periodos por turno.' });
    ...
    if (exists && exists.id) {
      await pool.query(
        `UPDATE escola_configuracao_grade
         SET turnos = ?, dias_semana = ?, periodos = ?, atualizado_em = NOW()
         WHERE escola_id = ?`,
        [turnosJson, diasJson, periodosJson, escola_id]
      );
      return res.json({ ok: true, atualizado: true, id: exists.id });
    } else {
      const [ins] = await pool.query(
        `INSERT INTO escola_configuracao_grade
           (escola_id, turnos, dias_semana, periodos, criado_em, atualizado_em)
         VALUES (?, ?, ?, ?, NOW(), NOW())`,
        [escola_id, turnosJson, diasJson, periodosJson]
      );
...
// Utilitario: parse seguro de campo JSON do banco
