...
router.put("/:id/vincular-professor", verificarEscola, async (req, res) => {
  ...
  try {
    ...
    const [[loteRow]] = await pool.query(
      `SELECT l.id, l.liberado_correcao, a.status AS avaliacao_status, a.id AS avaliacao_id
    ...
    if (loteRow.avaliacao_status === "notas_importadas" && !loteRow.liberado_correcao) {
...
// ─── GET /api/gabarito-lotes/meu-lote?avaliacao_id=X ────────────────────────
// Professor consulta o lote da SUA turma para uma avaliação (para saber se está liberado)
router.get("/meu-lote", verificarEscola, async (req, res) => {
  const { escola_id } = req.user;
  const { avaliacao_id, turma_nome } = req.query;
  ...
  try {
    let sql = `
      SELECT l.id, l.turma_nome, l.status, l.liberado_correcao,
             l.total_arquivos, l.total_corrigidos, l.avaliacao_id
      FROM gabarito_lotes l
      WHERE l.avaliacao_id = ? AND l.escola_id = ?
    `;
    const params = [avaliacao_id, escola_id];

    if (turma_nome) {
      sql += " AND l.turma_nome = ?";
    ...
    if (rows.length === 0) {
      return res.json({ lote: null, liberado_correcao: false });
    }
    res.json({ lote: rows[0], liberado_correcao: !!rows[0].liberado_correcao });
  } catch (err) {
    console.error("Erro ao buscar meu-lote:", err);
    res.status(500).json({ error: "Erro ao buscar lote." });
  }
});

// ─── POST /api/gabarito-lotes/:id/liberar-correcao ───────────────────────────
// Coordenador/diretor libera uma turma específica para re-correção após importação
router.post("/:id/liberar-correcao", verificarEscola, async (req, res) => {
  const { escola_id } = req.user;
  const perfil = req.user.perfil || req.user.role;
  const loteId = req.params.id;

  const perfisPermitidos = ["coordenador", "diretor", "supervisor", "admin"];
  if (!perfisPermitidos.includes(perfil)) {
    return res.status(403).json({ error: "Apenas coordenador ou direção podem liberar turmas para re-correção." });
  }

  try {
    const [[lote]] = await pool.query(
      `SELECT l.id, l.turma_nome, l.avaliacao_id, av.status AS avaliacao_status
       FROM gabarito_lotes l
       JOIN gabarito_avaliacoes av ON av.id = l.avaliacao_id
       WHERE l.id = ? AND l.escola_id = ?`,
      [loteId, escola_id]
    );
    ...
    if (lote.avaliacao_status !== "notas_importadas") {
      return res.status(409).json({
        error: "A turma só pode ser liberada para re-correção após a importação das notas.",
        avaliacao_status: lote.avaliacao_status,
      });
    }

    await pool.query(
      "UPDATE gabarito_lotes SET liberado_correcao = 1 WHERE id = ? AND escola_id = ?",
      [loteId, escola_id]
    );

    console.log(`[liberar-correcao] Lote #${loteId} (${lote.turma_nome}) liberado por ${perfil}`);
    res.json({ ok: true, lote_id: loteId, turma_nome: lote.turma_nome, liberado_correcao: true });
  } catch (err) {
    console.error("Erro ao liberar turma:", err);
    res.status(500).json({ error: "Erro ao liberar turma para re-correção." });
  }
});

// ─── POST /api/gabarito-lotes/:id/bloquear-correcao ──────────────────────────
// Coordenador/diretor re-bloqueia manualmente uma turma
router.post("/:id/bloquear-correcao", verificarEscola, async (req, res) => {
  const { escola_id } = req.user;
  const perfil = req.user.perfil || req.user.role;
  const loteId = req.params.id;
  ...
  const perfisPermitidos = ["coordenador", "diretor", "supervisor", "admin"];
  if (!perfisPermitidos.includes(perfil)) {
    return res.status(403).json({ error: "Apenas coordenador ou direção podem bloquear turmas." });
  }

  try {
    const [result] = await pool.query(
      "UPDATE gabarito_lotes SET liberado_correcao = 0 WHERE id = ? AND escola_id = ?",
      [loteId, escola_id]
    );
    ...
    res.json({ ok: true, lote_id: loteId, liberado_correcao: false });
  } catch (err) {
    console.error("Erro ao bloquear turma:", err);
    res.status(500).json({ error: "Erro ao bloquear turma." });
  }
});
...
// ─── DELETE /api/gabarito-lotes/arquivos/:id ─────────────────────────────────
// Exclui um arquivo/gabarito individual do lote (coordenador)
// Limpa: gabarito_arquivos, gabarito_ajustes_manuais, gabarito_respostas (para este arquivo)
// Decrementa total_arquivos do lote e, se lote ficou vazio, o deleta também
router.delete("/arquivos/:id", verificarEscola, async (req, res) => {
  const { escola_id } = req.user;
  const arquivoId = req.params.id;

  try {
    // 1. Buscar arquivo (verificar ownership e obter path para deletar do Spaces)
    const [[arq]] = await pool.query(
      `SELECT a.id, a.lote_id, a.arquivo_path, a.codigo_aluno, a.status,
              l.avaliacao_id, l.liberado_correcao, av.status AS avaliacao_status
       FROM gabarito_arquivos a
       JOIN gabarito_lotes l ON l.id = a.lote_id
       JOIN gabarito_avaliacoes av ON av.id = l.avaliacao_id
       WHERE a.id = ? AND a.escola_id = ?`,
    ...
    // 2. Bloquear exclusão se notas já foram importadas E a turma NÃO foi liberada para re-correção
    if (arq.avaliacao_status === "notas_importadas" && !arq.liberado_correcao) {
      return res.status(409).json({
        error: "Não é possível excluir gabaritos após a importação das notas. Solicite ao coordenador que libere a turma para re-correção.",
        codigo: "NOTAS_JA_IMPORTADAS",
      });
    }

    // 3. Deletar resposta de gabarito associada (se corrigido e identificado via QR)
    // gabarito_respostas usa (avaliacao_id, escola_id, codigo_aluno) como chave — sem arquivo_id
    if (arq.codigo_aluno && !arq.codigo_aluno.startsWith("ARQ_")) {
      await pool.query(
        "DELETE FROM gabarito_respostas WHERE avaliacao_id = ? AND escola_id = ? AND codigo_aluno = ?",
        [arq.avaliacao_id, escola_id, arq.codigo_aluno]
      );
    }

    // 4. Deletar ajustes manuais associados
    await pool.query(
      "DELETE FROM gabarito_ajustes_manuais WHERE arquivo_id = ?",
      [arquivoId]
    );

    // 5. Deletar o arquivo do banco
    await pool.query(
      "DELETE FROM gabarito_arquivos WHERE id = ? AND escola_id = ?",
      [arquivoId, escola_id]
    );

    // 6. Decrementar total_arquivos do lote (e recalcular total_corrigidos)
    await pool.query(
      `UPDATE gabarito_lotes SET
         total_arquivos = GREATEST(0, total_arquivos - 1),
         total_corrigidos = (
           SELECT COUNT(*) FROM gabarito_arquivos
           WHERE lote_id = ? AND escola_id = ? AND status = 'corrigido'
         ),
         status = CASE
           WHEN (SELECT COUNT(*) FROM gabarito_arquivos WHERE lote_id = ? AND escola_id = ?) = 0 THEN 'pendente'
           WHEN (SELECT COUNT(*) FROM gabarito_arquivos WHERE lote_id = ? AND escola_id = ? AND status != 'corrigido') = 0 THEN 'finalizado'
           ELSE 'em_correcao'
         END
       WHERE id = ? AND escola_id = ?`,
      [arq.lote_id, escola_id, arq.lote_id, escola_id, arq.lote_id, escola_id, arq.lote_id, escola_id]
    );

    // 7. Tentar remover do Spaces (não-bloqueante)
    try {
      const { deleteObjectFromSpaces } = await import("../storage/spacesUpload.js");
      if (arq.arquivo_path && (arq.arquivo_path.startsWith("uploads/") || arq.arquivo_path.includes("/gabaritos/"))) {
        await deleteObjectFromSpaces(arq.arquivo_path);
      }
    } catch (spacesErr) {
      // Não falhar por erro no Spaces — o registro já foi removido do BD
      console.warn(`[excluir-arquivo] Aviso: não foi possível deletar do Spaces: ${spacesErr.message}`);
    }

    res.json({ ok: true, message: "Arquivo excluído com sucesso.", lote_id: arq.lote_id });
  } catch (err) {
...
// ─── HELPER: Recalcular Nota do Arquivo ──────────────────────────────────────
async function recalcularNotaArquivo(arquivo_id, escola_id) {
  // 1. Buscar arquivo + avaliação_id
  const [arqRows] = await pool.query(
    `SELECT a.*, l.avaliacao_id
     FROM gabarito_arquivos a
     JOIN gabarito_lotes l ON l.id = a.lote_id
  ...
  // 2. Buscar gabarito oficial + configurações
  const [avRows] = await pool.query(
    `SELECT gabarito_oficial, num_questoes, nota_total, disciplinas_config, questoes_canceladas
     FROM gabarito_avaliacoes WHERE id = ? AND escola_id = ?`,
    [arq.avaliacao_id, escola_id]
  ...
  const cancelMap = {};
  for (const c of canceladas) cancelMap[c.numero] = c.modo;

  const numDesconsideradas  = canceladas.filter(c => c.modo === "desconsiderar").length;
  const numQuestoesEfetivas = Math.max(1, numQuestoes - numDesconsideradas);

  // 3. Buscar TODOS os ajustes manuais aprovados para este arquivo
  ...
  const ajustesMap = {};
  for (const a of todosAjustes) ajustesMap[a.questao_numero] = a.tipo_ajuste;

  // 4. Recalcular detalhes por questão
  const detalhes = [];
  for (let i = 0; i < numQuestoes; i++) {
    const questaoNum     = i + 1;
    const modoCancelada  = cancelMap[questaoNum];
  ...
  if (disciplinasConfig.length > 0) {
    acertosPorDisciplina = disciplinasConfig.map(dc => {
      const questoesDisciplina = detalhes.filter(d => d.numero >= dc.de && d.numero <= dc.ate);
      return {
        nome: dc.nome, disciplina_id: dc.disciplina_id, de: dc.de, ate: dc.ate,
        total: questoesDisciplina.length,
        acertos: questoesDisciplina.filter(d => d.acertou).length,
      };
    });
  }

  // 6. Atualizar gabarito_arquivos
  await pool.query(
    `UPDATE gabarito_arquivos SET acertos = ?, nota = ? WHERE id = ?`,
    [acertos, nota, arquivo_id]
  );

  // 7. Atualizar gabarito_respostas
  ...
  console.log(`[recalcularNota] arquivo ${arquivo_id} aluno ${codigoAluno} → ${acertos}/${numQuestoesEfetivas} acertos, nota ${nota}`);
  return { acertos, numQuestoesEfetivas, nota, notaTotal };
}
...
router.post("/arquivos/:id/ajuste-manual", verificarEscola, async (req, res) => {
  ...
  try {
    ...
    // Upsert: professor agora salva o ajuste que já é aprovado imediatamente
    await pool.query(
      `INSERT INTO gabarito_ajustes_manuais
        (arquivo_id, escola_id, questao_numero, tipo_ajuste, justificativa, professor_id, status, decidido_em)
    ...
    // Recalcular a nota imediatamente, já que o ajuste tem status aprovado
    const recalc = await recalcularNotaArquivo(arquivoId, escola_id);
    ...
    res.json({ ok: true, ajuste: ajuste[0], recalculado: true, notasAtualizadas: recalc });
  } catch (err) {
