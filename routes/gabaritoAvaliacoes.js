...
router.post("/", async (req, res) => {
  try {
    ...
    // Validações
    if (!data_aplicacao) {
      return res.status(400).json({ error: "A data de aplicação da prova é obrigatória." });
    }
    ...
    // aprovado/enviado do mesmo bimestre na escola, MAS apenas para as disciplinas aplicáveis.
    if (data_aplicacao && bimestre) {
      try {
        let disciplinasIn = [];
        if (disciplinas_config && Array.isArray(disciplinas_config)) {
          const ids = disciplinas_config.map(d => d.disciplina_id).filter(Boolean);
          if (ids.length > 0) {
            const [rows] = await pool.query('SELECT nome FROM disciplinas WHERE id IN (?)', [ids]);
            disciplinasIn = rows.map(r => r.nome);
          }
        }

        if (disciplinasIn.length > 0) {
          const [propagado] = await pool.query(
            `UPDATE itens_avaliacao ia
             ...
             WHERE pa.escola_id = ?
               ...
               AND pa.disciplina IN (?)`,
            [data_aplicacao, data_aplicacao, escola_id, `%${bimestre.replace(/[^\d]/g, '')}%`, disciplinasIn]
          );
...
router.put("/:id", async (req, res) => {
  try {
    ...
    // ── PROPAGA data_aplicacao na EDIÇÃO (se alterada) ──────────────
    if (req.body.data_aplicacao !== undefined) {
       if (r.data_aplicacao && r.bimestre) {
         try {
           let disciplinasIn = [];
           const dConfig = typeof r.disciplinas_config === 'string' ? JSON.parse(r.disciplinas_config) : r.disciplinas_config;
           if (dConfig && Array.isArray(dConfig)) {
             const ids = dConfig.map(d => d.disciplina_id).filter(Boolean);
             if (ids.length > 0) {
               const [rows] = await pool.query('SELECT nome FROM disciplinas WHERE id IN (?)', [ids]);
               disciplinasIn = rows.map(row => row.nome);
             }
           }
           if (disciplinasIn.length > 0) {
             const [propagado] = await pool.query(
               `UPDATE itens_avaliacao ia
                ...
                WHERE pa.escola_id = ?
                  ...
                  AND ia.fixo_direcao = 1
                  AND pa.disciplina IN (?)`,
               [r.data_aplicacao, r.data_aplicacao, escola_id, `%${r.bimestre.replace(/[^\d]/g, '')}%`, disciplinasIn]
             );
             console.log(`[gabarito] EDIÇÃO: Propagou data_aplicacao ${r.data_aplicacao} para ${propagado.affectedRows} item(ns) fixo_direcao`);
           }
         } catch (propErr) {
           console.warn('[gabarito] Propagação de data_aplicacao (edição) falhou:', propErr.message);
         }
       }
    }
    res.json({
...
router.post("/:id/importar-notas", async (req, res) => {
  ...
  try {
      ...
      for (const dc of discConfig) {
        ...
        const [result] = await conn.query(
          `INSERT INTO notas_diario
             (escola_id, plano_id, turma_id, aluno_id, item_idx, oportunidade_idx, nota, cor, updated_at)
           VALUES (?, ?, ?, ?, 0, 0, ?, NULL, NOW())
           ON DUPLICATE KEY UPDATE nota = VALUES(nota), cor = NULL, updated_at = NOW()`,
    ...
    // ── 10. Atualizar status da avaliação + bloquear todos os lotes ───────────
    await conn.query(
      "UPDATE gabarito_avaliacoes SET status = 'notas_importadas' WHERE id = ? AND escola_id = ?",
      [id, escola_id]
    );
    // Garante que todas as turmas voltam ao bloqueio (inclusive as liberadas para re-correção)
    await conn.query(
      "UPDATE gabarito_lotes SET liberado_correcao = 0 WHERE avaliacao_id = ? AND escola_id = ?",
      [id, escola_id]
    );
    ...
    // Mensagem reflete o estado real: sucesso total, parcial ou com avisos
    let message;
    ...
    } else if (temErros && alunosProcessados.length === 0) {
      message = `⚠️ Nenhum aluno foi importado. Verifique os erros abaixo.`;
    } else {
      const partes = [];
      if (alunosProcessados.length > 0) partes.push(`${alunosProcessados.length} aluno(s) importado(s)`);
      if (temErros)  partes.push(`${erros.length} não importado(s) — sem identificação QR`);
      if (temAvisos) partes.push(`${avisos.length} combinação(ões) disciplina/turma sem PAP`);
      message = `⚠️ Importação parcial: ${partes.join(" · ")}.`;
    }

    res.json({
      success: true,
      parcial: temErros || temAvisos,   // flag explícita para o frontend exibir alerta
      message,
      resumo: {
