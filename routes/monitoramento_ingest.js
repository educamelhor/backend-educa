...
// Controle de persistência (evita piscar rosto na tela em falhas de 1 frame)
const _facesLastNonEmpty = new Map(); // key: escolaId_cameraId -> timestamp
...
function safeAtomicWriteRename(tmpPath, finalPath, data, isBuffer = false) {
  if (isBuffer) {
  ...
  try {
    ...
    if (err.code === "EPERM" || err.code === "EBUSY") {
      try {
        ...
        // Ignora se estiver totalmente lockado; o próximo frame atualiza.
      }
    } else {
      throw err;
    }
...
router.get("/snapshot/:camera_id.jpg", (req, res) => {
  ...
  const escolaDirRaw = String(
    req.query.escola_dir || req.header("x-escola-dir") || ""
  ).trim();

  if (!escolaDirRaw) return res.status(400).json({ ok: false, message: "escola_dir obrigatório" });

  const normalizar = (s) => s.toLowerCase().replace(/[-_]/g, "");
  const escolaDirNorm = normalizar(escolaDirRaw);
  ...
  try {
    ...
    const candidates = entries.filter(e => normalizar(e) === escolaDirNorm);
    for (const dir of candidates) {
      const p = path.join(uploadsBase, dir, "monitoramento", `camera-${camPad}`, "frame.jpg");
      if (fs.existsSync(p)) {
        framePath = p;
        break;
      }
    }
    if (!framePath && candidates.length > 0) {
      framePath = path.join(uploadsBase, candidates[0], "monitoramento", `camera-${camPad}`, "frame.jpg");
    }
  } catch (_) {}

  if (!framePath || !fs.existsSync(framePath)) {
    return res.status(404).json({ ok: false, message: "Frame não disponível", frame_path: framePath || escolaDirRaw });
  }
...
// ─────────────────────────────────────────────────────────────────────────────
// GET /api/monitoramento/ingest/faces/:camera_id.json
// Serve o faces.json do disco — público (sem JWT), mesmo matching do snapshot.
// ─────────────────────────────────────────────────────────────────────────────
router.get("/faces/:camera_id.json", (req, res) => {
  const camId = toNumber(req.params.camera_id, 0);
  if (!camId) {
    res.setHeader("Content-Type", "application/json");
    return res.end(JSON.stringify({ ok: false, message: "camera_id inválido" }));
  }
  ...
  if (!escolaDirRaw) {
    res.setHeader("Content-Type", "application/json");
    return res.end(JSON.stringify({ ok: false, message: "escola_dir obrigatório" }));
  }
  ...
  try {
    ...
    const candidates = entries.filter(e => normalizar(e) === escolaDirNorm);
    for (const dir of candidates) {
      const p = path.join(uploadsBase, dir, "monitoramento", `camera-${camPad}`, "faces.json");
      if (fs.existsSync(p)) {
        facesPath = p;
        break;
      }
    }
    if (!facesPath && candidates.length > 0) {
      facesPath = path.join(uploadsBase, candidates[0], "monitoramento", `camera-${camPad}`, "faces.json");
    }
  } catch (_) {}

  if (!facesPath || !fs.existsSync(facesPath)) {
    return res.end(JSON.stringify({ ok: true, faces: [], width: 0, height: 0 }));
  }

  try {
    const buf = fs.readFileSync(facesPath);
    return res.end(buf);
  } catch (err) {
    return res.end(JSON.stringify({ ok: true, faces: [], width: 0, height: 0, error: String(err?.message) }));
  }
});
...
router.post("/frame", validarTokenWorker, exigirEscolaId, async (req, res) => {
  try {
    ...
    safeAtomicWriteRename(tmpPath, finalPath, jpegBuf, true);
...
router.post("/faces", validarTokenWorker, exigirEscolaId, async (req, res) => {
  try {
    ...
    const cacheKey = `${escola_id}_${req.body?.camera_id || 0}`;
    if (facesIn.length > 0) {
      _facesLastNonEmpty.set(cacheKey, Date.now());
    } else {
      const lastT = _facesLastNonEmpty.get(cacheKey) || 0;
      if (Date.now() - lastT < 2500) {
        // Ignora gravação de faces=[] se a última não-vazia foi há menos de 2.5s
        return res.json({ ok: true, ignored: true, reason: "persistence" });
      }
    }
    ...
    const faces = facesIn
      .map((f) => {
        ...
        // ✅ Preserva dados de reconhecimento enviados pelo worker (TIER 2)
        // Bug anterior: sempre sobrescrevia com recognized:false/"DESCONHECIDO"/0
        const recognized  = Boolean(f?.recognized);
        const aluno_nome  = String(f?.aluno_nome || f?.nome || "").trim();
        const turma       = String(f?.turma || "").trim();
        const score       = recognized ? toNumber(f?.score, 0) : 0;

        return {
          bbox: { x, y, width, height },
          recognized,
          aluno_nome: recognized ? aluno_nome : "",
          turma:      recognized ? turma : "",
          score,
        };
      })
      .filter(Boolean);
    ...
    if (debugFaces) {
      try {
        ...
        safeAtomicWriteRename(debugTmp, debugPath, JSON.stringify(debugOut, null, 2));
      } catch (e) {
        console.warn("[ingest/faces][debug] falhou ao gravar faces.last_payload.json", e?.message);
      }
    }

    // Escrita atômica segura (trata EPERM no Windows)
    safeAtomicWriteRename(tmpPath, facesPath, JSON.stringify(out));
