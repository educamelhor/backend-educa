...
function assertBuffer(buf) {
  if (!buf || !(buf instanceof Buffer) || buf.length === 0) {
    throw new Error("Arquivo inv├ílido (buffer vazio).");
  }
}
...
async function cropAvatarBuffer(inputBuffer, inputMimeType) {
  ...
  if (!Number.isFinite(size) || size < 128 || size > 2048) {
    throw new Error("CAPTURE_CROP_SIZE inv├ílido (128 a 2048).");
  }

  const mime = String(inputMimeType || "").toLowerCase();

  // rotate() corrige orienta├º├úo EXIF (muito comum em mobile)
  let img = sharp(inputBuffer, { failOnError: false }).rotate().resize(size, size, {
    fit: "cover",
    position: "attention", // heur├¡stica que tende a ÔÇ£puxarÔÇØ para regi├úo mais relevante (rosto geralmente)
  });
...
export async function uploadImageBufferToSpaces({
  ...
  const cmd = new PutObjectCommand({
    ...
    // Ô£à Necess├írio para renderiza├º├úo direta no EDUCA.MELHOR (evita AccessDenied no browser)
    ACL: "public-read",
  });

  const out = await s3.send(cmd);

  // URL padr├úo do Spaces (virtual hosted style)
...
export async function deleteObjectFromSpaces(objectKey) {
  ...
  if (!key) {
    throw new Error("objectKey obrigat├│rio para delete no Spaces.");
  }
...
export async function getSignedGetObjectUrl(objectKey, expiresInSeconds = 3600) {
  ...
  if (!key) {
    throw new Error("objectKey obrigat├│rio para assinar URL.");
  }

  const expires = Number(expiresInSeconds);
  if (!Number.isFinite(expires) || expires <= 0 || expires > 60 * 60 * 24) {
    throw new Error("expiresInSeconds inv├ílido (1s a 86400s).");
  }
...
/**
 * Upload gen├®rico de arquivo (sem crop/resize) para DigitalOcean Spaces.
 * ├Ütil para gabaritos, PDFs, documentos, etc.
 *
 * @param {Object} args
 * @param {Buffer} args.buffer
 ...
 * ├Ütil para enviar ao OMR para processamento.
 *
...
export async function downloadBufferFromSpaces(objectKey) {
  ...
  const key = String(objectKey || "").trim().replace(/^\/+/, "");
  if (!key) {
    throw new Error("objectKey obrigat├│rio para download do Spaces.");
  }
