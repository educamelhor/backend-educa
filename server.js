...
import escolaConfiguracaoGradeRouter from "./routes/escola-configuracao-grade.js";
...
async function bootstrap() {
    ...
    try {
      ...
      if (colsAPH.length === 0) {
        await pool.query("ALTER TABLE aph_atendimentos ADD COLUMN sinais_pa VARCHAR(50) DEFAULT NULL, ADD COLUMN sinais_fc VARCHAR(50) DEFAULT NULL, ADD COLUMN sinais_temperatura VARCHAR(50) DEFAULT NULL");
        console.log("[MIGRATION] Colunas PA, FC e Temperatura adicionadas em aph_atendimentos");
      }
    } catch (migErr) {
      console.warn("[MIGRATION] Erro ao aplicar migration APH Sinais (nao critico):", migErr.message);
    }
  ...
  // app.use("/api/app-pais", appPaisRouterModule) não roteia POSTs corretamente.
  // Solução: router separado (mesmo padrão de appAlunoAuthRouter) montado ANTES
  // de qualquer middleware autenticado.
  // Rotas: /solicitar-codigo, /verificar-codigo, /salvar-email, /salvar-telefone,
  //        /credencial/contexto, /credencial/pre-cadastro, /credencial/solicitar
  ...
  //    - mountToApp registra rotas DIRETO no app (workaround Express 5 + Docker).
  //    - O router tem o 1º GET /boletim corrompido (arquivo concatenado acidentalmente)
  //      que causa 500 "Erro ao registrar token do dispositivo" se atingido primeiro.
  //    - Montando mountToApp ANTES, o 2º handler /boletim (correto) ganha prioridade.
  mountAppPaisToApp(app, "/api/app-pais");
  console.log("[FF] FF_APP_PAIS: mountToApp registrado diretamente no app ✅ (priority fix /boletim)");

  // 2) app.use como fallback (Express 5 não roteia POSTs pelo router em Docker)
  app.use("/api/app-pais", appPaisRouterModule);
  console.log("[FF] FF_APP_PAIS: router montado via app.use ✅ stack:", appPaisRouterModule.stack?.length);
  ...
  // Rota pública para verificar carteirinha
  app.use("/public", carteirinhaRouter);
  console.log("[CARTEIRINHA] router montado em /public ✅");
  ...
  if (FF_HORARIOS) {
    ...
    app.use(
      "/api/escola/configuracao-grade",
      autenticarToken,
      verificarEscola,
      escolaConfiguracaoGradeRouter
    );
    app.use(
      "/api/preferencias",
      autenticarToken,
