...
async function bootstrap() {
  ...
  // GET  /api/admin/fix-disciplina-id?escola_id=3&dry=true&secret=XXX  → preview
  // POST /api/admin/fix-disciplina-id?escola_id=3&secret=XXX           → executa
  // Remove após confirmar correção dos dados.
  // ─────────────────────────────────────────────────────────────────────────
  ...
  async function fixDisciplinaId(req, res) {
    ...
    try {
      // 1. Buscar todas as disciplinas ativas da escola (fonte da verdade)
      const [disciplinasAtivas] = await pool.query(
        "SELECT id, nome, etapa, turno FROM disciplinas WHERE escola_id = ?",
        [escolaId]
      );

      // 2. Buscar todas as notas da escola com nome da disciplina via JOIN
      const [notasComNome] = await pool.query(
        `SELECT n.id AS nota_id, n.disciplina_id AS id_atual,
                d.nome AS nome_atual, d.id AS id_tabela
         FROM notas n
      ...
      // 3. Montar mapa: nome (upper+trim) → id ativo da escola
      const mapaIdPorNome = {};
      for (const disc of disciplinasAtivas) {
        const key = disc.nome.trim().toUpperCase();
        // Preferir a disciplina que já existe — usa o primeiro encontrado por nome
        if (!mapaIdPorNome[key]) mapaIdPorNome[key] = disc.id;
      }

      // 4. Identificar notas com ID divergente
      const divergentes = notasComNome.filter((n) => {
        if (!n.nome_atual) return false; // disciplina deletada — ignorar
        const key = n.nome_atual.trim().toUpperCase();
        const idCorreto = mapaIdPorNome[key];
        return idCorreto && idCorreto !== n.id_atual;
      });

      if (divergentes.length === 0) {
        return res.json({ ok: true, message: "Nenhuma divergência encontrada.", total: 0 });
      }
      ...
      if (dryRun) {
        return res.json({ dry: true, total: preview.length, divergentes: preview });
      }
