...
async function selecionarNgSelect(page, placeholder, valor, timeout = 8000, fuzzyFn = null) {
  ...
  // ── Estratégia 0: Já está selecionado? ────────────────────────────────────
  const alreadySelected = await ng.locator('.ng-value-label').textContent().catch(() => '');
  if (alreadySelected && isMatch(alreadySelected)) {
    console.log(`[educadf.pap] ⚡ ng-select "${placeholder}" já estava pré-selecionado: "${alreadySelected.trim()}"`);
    return true;
  }
...
export async function exportarNotasEducaDF(session, credenciais, plano) {
  ...
  console.log(`  Colunas:  ${plano.colunas?.map(c => c.nomeColuna).join(', ') || plano.nomeColuna}`);
  ...
  try {
    ...
    if (!calConf.ok) {
      console.warn(
        `[educadf.notas] ⚠️ Calendário não confirmou a turma "${plano.turmas}" após Filtrar (ignorando e prosseguindo). ` +
        `Motivo: ${calConf.motivo} | ${calConf.total} eventos.`
      );
    } else {
    ...
    // ════════════════════════════════════════════════════════════════════
    // PASSO 7 — INSERIR NOTAS NAS COLUNAS
    // ════════════════════════════════════════════════════════════════════
    const colunasParaExportar = plano.colunas || (plano.nomeColuna && plano.alunos ? [{ nomeColuna: plano.nomeColuna, alunos: plano.alunos }] : []);
    ...
    console.log(`[educadf.notas] Coluna "${targetColuna}" → td[${colunaIdx.idx}] via ${colunaIdx.via || 'global'}`);
    ...
    if (colunaIdx.idx < 0) {
      ...
      console.error(
        `[educadf.notas] ❌ Coluna "${targetColuna}" não encontrada na tabela. ` +
        `Motivo: ${colunaIdx.motivo}. ` +
        `Cabeçalhos disponíveis: ${dispStr}`
      );
      totalErros += alunos.length;
      continue;
    }
    ...
    if (colunaIdx.confirmOk === false) {
      console.error(
        `[educadf.notas] ❌ VALIDAÇÃO CRUZADA FALHOU: esperava coluna "${targetColuna}" ` +
        `mas td[${colunaIdx.idx}] corresponde ao th "${colunaIdx.confirmTh}". ` +
        `Pulando esta coluna para evitar lançar notas no lugar errado.`
      );
      totalErros += alunos.length;
      continue;
    }

    // Pré-carrega todos os REs visíveis na tabela apenas uma vez
    if (c === 0) {
    ...
    for (const aluno of alunos) {
      ...
      const linhaInfo = await page.evaluate(({ re, nome, colIdx, colName }) => {
        ...
        for (let rIdx = 0; rIdx < rows.length; rIdx++) {
          ...
          if (!tdNota) return { ok: false, motivo: `célula ${colIdx} não existe na linha (max=${cells.length})` };

          const input = tdNota.querySelector('input[type="text"], input[type="number"], input');
          if (!input) return { ok: false, motivo: `sem input na célula ${colIdx}` };

          // Detecta campo desabilitado
          if (input.disabled || input.hasAttribute('disabled')) {
            return {
              ok:       false,
              disabled: true,
              motivo:   `campo desabilitado no EDUCADF para a coluna ${colName} (aluno ausente no dia)`,
            };
          }
      ...
      }, { re: aluno.re, nome: aluno.nome, colIdx: colunaIdx.idx, colName: targetColuna });
